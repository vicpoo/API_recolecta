...
func (r *EmpleadoPostgresRepository) Create(ctx context.Context, empleado *entities.Empleado) (int, error) {
	const q = `
		INSERT INTO empleado (nombre, apellidos, mail, password, username, desactivado, rol_id, created_at, updated_at)
...
func (r *EmpleadoPostgresRepository) Update(ctx context.Context, empleado *entities.Empleado) error {
	const q = `
		UPDATE empleado
		SET nombre = $1,
		    apellidos = $2,
...
func (r *EmpleadoPostgresRepository) Delete(ctx context.Context, id int) error {
	const q = `
		UPDATE empleado
		SET deleted_at = NOW(),
		    updated_at = NOW(),
